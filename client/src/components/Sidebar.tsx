...
export default function Sidebar({ isMobileDrawer = false }: SidebarProps) {
  ...
  const { isOpen, toggle, closeMobile } = useSidebar();
  const { user, logoutMutation } = useAuth();
  const { t } = useLanguage();

  const isExpanded = isMobileDrawer ? true : isOpen;
  ...
  return (
    <aside 
      className={`${
        isMobileDrawer
          ? "w-full h-full flex flex-col justify-between select-none bg-card dark:bg-black"
          : `fixed left-0 top-0 bottom-0 z-50 h-screen transition-all duration-300 ease-in-out border-r border-border bg-card dark:bg-black flex flex-col justify-between select-none ${
    ...
    >
      ...
      <div className="flex flex-col min-h-0 flex-1">
        <div className={`flex items-center h-16 px-4 border-b border-border shrink-0 ${
          ...
          {isExpanded ? (
            <Link href="/dashboard" onClick={handleItemClick} className="flex items-center gap-2 cursor-pointer">
      ...
      <div className="p-3 border-t border-border shrink-0 flex flex-col gap-2 bg-card dark:bg-black">
        ...
        {isExpanded && isFreeTier && (
          <Link href="/pricing" onClick={handleItemClick}>
        ...
        {user && (
          <div className={`flex items-center ${
            isExpanded ? "justify-between gap-3 px-2 py-1.5 rounded-xl bg-muted/30 border border-border/40" : "justify-center py-1"
          }`}>
            <Link href="/settings" onClick={handleItemClick} className="flex items-center gap-3 cursor-pointer min-w-0 flex-1">
